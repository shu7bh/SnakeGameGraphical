...
Game::Game( MainWindow& wnd )
	...
	gfx( wnd )
{
	walls.push_back(std::make_unique<Wall>(0, 0, 0, 0, 0, 10, gfx.ScreenHeight));
	walls.push_back(std::make_unique<Wall>(0, 0, 0, 0, 0, gfx.ScreenWidth, 10));
	walls.push_back(std::make_unique<Wall>(0, gfx.ScreenHeight - 10 - 1, 0, 0, 0, gfx.ScreenWidth, 10));
	walls.push_back(std::make_unique<Wall>(gfx.ScreenWidth - 10 - 1, 0, 0, 0, 0, 10, gfx.ScreenHeight));

	snake.push_back(std::make_unique<SnakeHead>(300, 200));
	//foods.push_back(addEntity(255,100,100));
	for (auto i = foods.size(); i <= int(foodCount); ++i)
		foods.push_back(addEntity(255, 100, 100));
...
void Game::UpdateModel()
{
	if (!isGameOver)
	{
		for (auto i = walls.size(); i <= int(wallCount); ++i)
			walls.push_back(addEntity(0, 0, 0));

		snake.front()->changeDir(wnd);

		for (auto i = snake.size() - 1; i; --i)
			if (snake[i]->DelayChange() <= 0)
				snake[i]->setSVX(snake[i - 1]->VX()), snake[i]->setSVY(snake[i - 1]->VY());

		for (auto& snake : snake)
			snake->update();

		for (const auto& wall : walls)
			if (Entity::checkCollision(*snake.front(), *wall))
				isGameOver = true;

		for (auto i = 2; i < snake.size(); ++i)
			if (Entity::checkCollision(*snake.front(), *(snake[i])))
				isGameOver = true;

		for (const auto& food : foods)
			if (Entity::checkCollision(*food, *snake.front()))
				food->isEaten = true;
...
void Game::ComposeFrame()
{
	for (auto i = 0; i < gfx.ScreenWidth; ++i)
		for (auto j = 0; j < gfx.ScreenHeight; ++j)
			gfx.PutPixel(i, j, 100, 255, 100);

	for (const auto& wall : walls)
		wall->draw(gfx);
	for (const auto& snake : snake)
		snake->draw(gfx);
	for (const auto& food : foods)
		food->draw(gfx);
...
std::unique_ptr<Entity> Game::addEntity(int r, int g, int b)
{
	bool notProper = true;
	int x, y;
	...
	static std::uniform_int_distribution<int> rx(75, gfx.ScreenWidth - 76), ry(75, gfx.ScreenHeight - 76);

	while (notProper) 
	{
		notProper = false;
		x = rx(generator), y = ry(generator);
		entity.setX(x), entity.setY(y);
		for (const auto& wall : walls)
			if (Entity::checkCollision(entity, *wall))
				notProper = true;
		for (const auto& snake : snake)
			if (Entity::checkCollision(entity, *snake))
				notProper = true;
		for (const auto& food : foods)
			if (Entity::checkCollision(entity, *food))
				notProper = true;
	}
	return std::make_unique<Entity>(x, y, r, g, b); 
}

void Game::lengthenSnake()
{
	int x = snake.front()->X(), y = snake.front()->Y(),
		vx = snake.front()->VX(), vy = snake.front()->VY(),
		delayChange = snake.front()->DelayChange();
	snake.pop_front();
	snake.push_front(std::make_unique<SnakeBody>(x, y, delayChange, vx, vy));
	int tx = (vx == 0) ? 0 : (vx > 0) ? 1 : -1;
	int ty = (vy == 0) ? 0 : (vy > 0) ? 1 : -1;
	snake.push_front(std::make_unique<SnakeHead>(x + 20 * tx, y + 20 * ty, delayChange, vx, vy));
}
